...
async def _handle_tool_calls(
    ...
    async def call_mcp_tool(tool_call):
        ...
        # If it's a database tool, inject the selected connection name.
        ...
        if tool_result.isError:
            ...
            # Try to get a more specific error message from the content
            ...
            if tool_result.structuredContent is not None:
                # For structured data (lists, dicts), JSON dump it.
                tool_content = json.dumps(tool_result.structuredContent)
            elif tool_result.content:
                # For unstructured data (strings), concatenate text blocks.
                # This handles tools returning simple strings like read_file.
                text_parts = [
                    block.text
                    for block in tool_result.content
                    if isinstance(block, types.TextContent)
                ]
                # We still JSON dump the final string so it's a valid JSON string literal for the LLM.
                tool_content = json.dumps("".join(text_parts))
            else:
                # If there's no content at all, return an empty string.
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    async with stack:
        try:
            ...
            # A more descriptive system prompt to guide the LLM's behavior and prevent loops.
...
For any non-database questions, you must answer directly without using any tools."""
            ...
            while True:
                ...
                use_mcp = payload.get("use_mcp", True)  # Default to True for safety
                ...
                # This inner loop handles a single conversational turn, which may involve
                # multiple back-and-forths with the LLM if tools are used.
                while True:
                    if llm_provider == "ollama":
                        client_to_use = ollama_client
                        model_to_use = llm_model or "llama3.1:latest"  # Default Ollama model
                    ...
                    if aggregated_tool_calls and use_mcp:
                        ...
                        # Continue the inner loop to send tool results back to the LLM
                        continue
                    else:
                        # No tool calls, so the turn is over.
                        # Send the complete response at once to ensure proper formatting.
